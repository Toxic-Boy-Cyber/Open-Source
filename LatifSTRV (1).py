...
def keli():
    ...
    gt = ['GT','SM','SAMSUNG']
    return str(rc([f"Mozilla/5.0 (Linux; U; Android {str(rr(4,9))}.{str(rr(1,9))}.{str(rr(1,9))}; {str(rc(kage))}; {str(rc(gt))}-{str(rc(sama))} Build/{str(rc(oo))}) AppleWebKit/534.30 (KHTML, like Gecko) Version/{str(rr(1,4))}.0 UCBrowser/{str(rr(1,9))}.0.0.{str(rr(99,999))} U3/0.{str(rr(1,9))}.0 Mobile Safari/534.30 AliApp(TB/{str(rr(1,9))}.{str(rr(1,9))}.{str(rr(1,9))}) WindVane/{str(rr(1,9))}.0.0 {str(rr(1000,1900))}X{str(rr(1000,1999))} GCanvas/{str(rr(1,9))}.{str(rr(1,9))}.{str(rr(1,9))}.{str(rr(19,99))}"]))
	else:redmi.append(F)
try:abcd = open('.proxy.txt','r').read().splitlines()
except:sys.exit(f" [{M}>{P}] gagal dump proxy")
print(' TOTAL PROXY : '+str(len(abcd)))
print(' TOTAL USER AGENT : '+str(len(redmi)))
sleep(1)
